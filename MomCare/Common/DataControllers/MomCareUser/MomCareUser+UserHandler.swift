...
extension MomCareUser {
    ...
    private func serializeAndPost<T: Codable, R: Codable & Sendable>(
        ...
        _ object: T, endpoint: Endpoint, onFailureMessage: String
    ...
    func createNewUser(_ user: User) async -> Bool {
        ...
        guard let response: CreateResponse = await serializeAndPost(user, endpoint: .register, onFailureMessage: "User registration failed."
    ...
    func loginUser(email: String, password: String) async -> Bool {
        ...
        guard let response: CreateResponse = await serializeAndPost(credentials, endpoint: .login, onFailureMessage: "Login failed for email: \(email)"
    ...
    @objc func refreshToken() async -> Bool {
        ...
        guard let email = KeychainHelper.get("emailAddress"), let password = KeychainHelper.get("password") else {
        ...
        guard let response: CreateResponse = await serializeAndPost(credentials, endpoint: Endpoint.refresh, onFailureMessage: "Token refresh failed."
    ...
    func updateUser(_ updatedUser: User?) async -> Bool {
        ...
        guard let response: UpdateResponse = await serializeAndPost(updatedUser, endpoint: .update, onFailureMessage: "User update failed."
    ...
    func updateUserMedical(_ medicalData: UserMedical) async -> Bool {
        ...
        guard let response: UpdateResponse = await serializeAndPost(medicalData, endpoint: .updateMedicalData, onFailureMessage: "User medical data update failed."
