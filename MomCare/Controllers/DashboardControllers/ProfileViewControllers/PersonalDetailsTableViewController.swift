...
class PersonalDetailsTableViewController: UITableViewController {
    ...
    func saveUser() async {
        ...
        if let heightText = userHeightButton.title(for: .normal)?.replacingOccurrences(of: " cm", with: ""), let height = Double(heightText) {

            MomCareUser.shared.user?.medicalData?.height = height

        }

        if let weightText = userCurrentWeightButton.title(for: .normal)?.replacingOccurrences(of: " kgs", with: ""), let weight = Double(weightText) {

            MomCareUser.shared.user?.medicalData?.currentWeight = weight

        }

        if let preWeightText = userPrePregnancyWeightButton.title(for: .normal)?.replacingOccurrences(of: " kgs", with: ""), let preWeight = Double(preWeightText) {
