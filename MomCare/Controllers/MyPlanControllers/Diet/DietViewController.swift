...
class DietViewController: UIViewController {
    ...
    private func updateProgress(
        ...
        bar: UIProgressView?, label: UILabel?, goal: Double, readFunction: @escaping () async -> Double
    ...
    private func prepareProgress() {
        ...
        updateProgress(
            ...
            bar: proteinProgressBar, label: proteinProgressLabel, goal: plan.totalProtien, readFunction: { await HealthKitHandler.shared.readTotalProtein() }

        )

        updateProgress(

            bar: carbsProgressBar, label: carbsProgressLabel, goal: plan.totalCarbs, readFunction: { await HealthKitHandler.shared.readTotalCarbs() }

        )

        updateProgress(

            bar: fatsProgressBar, label: fatsProgressLabel, goal: plan.totalFat, readFunction: { await HealthKitHandler.shared.readTotalFat() }
