...
struct ExerciseProgressView: View {
    ...
    private func exerciseIconSection(for exercise: Exercise, isBreathing: Bool) -> some View {
        ExerciseIconView(exercise: exercise, isBreathing: isBreathing)
    }
...
struct ExerciseIconView: View {
    ...
    var body: some View {
        ...
        .task {
            if let imageUri = exercise.imageUri {
                exerciseImage = await UIImage().fetchImage(from: imageUri)
            }
        }
    }
}
